...
import common
from real_estate_simulator import MAX_CONTRIBUTION, MAX_MONTHLY_PAYMENT, RealEstateError, RealEstatePurchaseSimulator
...
class SimulatorInterface(QObject):
    ...
    def __init__(self):
        ...
        self.simulation_dict = {
            ...
            "house size": {self.simu_func: self.simulator.find_house_size, self.simu_result: "surface", self.unit_result: "m²"},
            "contribution": {self.simu_func: self.simulator.find_contribution, self.simu_result: "contribution", self.unit_result: "€"},
            "monthly payment": {self.simu_func: self.simulator.find_monthly_payment, self.simu_result: "monthly_payment", self.unit_result: "€/months"},
            "interest rate": {self.simu_func: self.simulator.find_interest_rate, self.simu_result: "interest_rate", self.unit_result: "%"},
        }
    ...
    def launch_simulation(self, target, simulation_parameters):
        ...
        match target:
            case "year":
                ...
                result = QCoreApplication.translate("Simulator interface", "{} years, {} months and {} days").format(year, month, day)
    ...
    def get_translate_year(self):
        ...
        if self.last_year_result is not None:
            text = QCoreApplication.translate("Simulator interface", "{} years, {} months and {} days").format(*self.last_year_result)
        return text
    ...
    def get_translated_model_error(self, error_pop_up_still_opened=False):
        ...
        return (
            QCoreApplication.translate("Simulator interface", "Model can not converge, maximum value {} is {} {}")
